...
import com.bumptech.glide.load.engine.DiskCacheStrategy; // Import added
...
public class TagEditorArtworkHelper {
    ...
    public TagEditorArtworkHelper(TagEditorActivity activity, 
    ...
    public void loadArtworkFromUri(Uri uri) {
        ...
        try {
            ...
            // 2. Load Bitmap for caching (Let Glide decide size, but SKIP CACHE to ensure freshness)
            Glide.with(activity)
                .asBitmap()
                .load(uri)
                .diskCacheStrategy(DiskCacheStrategy.NONE) // Skip disk cache
                ...
                .into(new CustomTarget<Bitmap>() {
                    ...
                    @Override public void onLoadCleared(Drawable p) {}
                });
            ...
            new Thread(() -> {
                try {
                    ...
                    // 4. Load Bytes
                    InputStream isBytes = activity.getContentResolver().openInputStream(uri);
                    if (isBytes == null) throw new java.io.IOException("InputStream is null for bytes: " + uri);

                    ByteArrayOutputStream baos = new ByteArrayOutputStream();
                    byte[] buffer = new byte[8192];
                    int read;
                    while ((read = isBytes.read(buffer)) != -1) baos.write(buffer, 0, read);
                    isBytes.close();
                    
                    selectedArtworkBytes = baos.toByteArray();
                    selectedArtworkMimeType = detectMimeType(selectedArtworkBytes, uri.toString());
                    
                    Log.d(TAG, "Successfully loaded artwork bytes. Size: " + selectedArtworkBytes.length + ", MIME: " + selectedArtworkMimeType);
                    ...
                    Log.e(TAG, "Failed to load image metadata/bytes from URI: " + uri, e);
                    e.printStackTrace();
                }
            }).start();
            ...
            Toast.makeText(activity, "Failed to load image: " + e.getMessage(), Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
        ...
        Glide.with(activity)
            ...
            .listener(new RequestListener<Drawable>() {
                ...
                public boolean onResourceReady(Drawable resource, Object model, Target<Drawable> target, DataSource dataSource, boolean isFirstResource) {
                    ...
                    updateRestoreStateCallback.run();
                    activity.hideLoading();
                    Toast.makeText(activity, "Metadata fetched!", Toast.LENGTH_SHORT).show();
                    return false;
                }
            }).into(artworkImageView);
    }
                ...
                public void onResourceReady(Bitmap resource, Transition<? super Bitmap> t) {
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            BitmapFactory.decodeByteArray(selectedArtworkBytes, 0, selectedArtworkBytes.length, options);
                            realWidth = options.outWidth;
                            realHeight = options.outHeight;
                            
                            activity.runOnUiThread(() -> updateArtworkDimensionsBadge());
                        } catch (Exception e) { 
                            Log.e(TAG, "Failed to download artwork bytes from URL: " + url, e);
                        }
                    }).start();
                }
                @Override public void onLoadCleared(Drawable p) {}
    ...
    // ... (detectMimeType, showArtworkOptionsDialog, etc. remain unchanged)
    ...
    public String detectMimeType(byte[] imageData, String imageUrl) {
        if (imageData != null && imageData.length >= 12) {
            ...
            if (imageData[0] == (byte) 0x89 && imageData[1] == 0x50 && imageData[2] == 0x4E && imageData[3] == 0x47) return "image/png";
    ...
    public void showArtworkOptionsDialog(String intentArtworkUrl) {
        ...
        final int currentW = (artworkChanged && realWidth > 0) ? realWidth : 
                           (artworkChanged ? selectedArtwork.getWidth() : originalArtwork.getWidth());
        final int currentH = (artworkChanged && realHeight > 0) ? realHeight : 
                           (artworkChanged ? selectedArtwork.getHeight() : originalArtwork.getHeight());
        ...
        resizeContainer.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
        ...
        widthInput.setLayoutParams(new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f));
        ...
        heightInput.setLayoutParams(new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f));
        ...
        boolean canResize = intentArtworkUrl != null && (intentArtworkUrl.contains("{w}") || intentArtworkUrl.contains("{h}"));
        if (!canResize) {
        ...
        Runnable checkValidation = () -> {
            ...
            if (w.equals("N/A") || h.equals("N/A")) {
                ...
                if (applyBtn != null) { applyBtn.setEnabled(false); applyBtn.setAlpha(0.5f); }
                return;
            }
            boolean isInvalid = w.isEmpty() || h.isEmpty() || w.equals("0") || h.equals("0");
            if (!isInvalid) {
                try { if (Integer.parseInt(w) == 0 || Integer.parseInt(h) == 0) isInvalid = true; } catch (NumberFormatException e) { isInvalid = true; }
            }
            saveToStorageBtn.setEnabled(!isInvalid);
            saveToStorageBtn.setAlpha(isInvalid ? 0.5f : 1.0f);
            Button applyBtn = dialog.getButton(AlertDialog.BUTTON_POSITIVE);
            if (applyBtn != null) { applyBtn.setEnabled(!isInvalid); applyBtn.setAlpha(isInvalid ? 0.5f : 1.0f); }
        };
        ...
        if (canResize) {
            ...
            TextWatcher tw = new TextWatcher() {
                ...
                public void afterTextChanged(Editable s) {
                    ...
                    if (s.length() == 0) {
                        isAutoUpdating = true;
                        if (widthInput.hasFocus()) heightInput.setText(""); else widthInput.setText("");
                    ...
                    try {
                        ...
                        if (widthInput.hasFocus()) {
                            ...
                            heightInput.setText(String.valueOf(Math.round(w / aspectRatio)));
                        } else {
        ...
        saveToStorageBtn.setOnClickListener(v -> {
            String wStr = widthInput.getText().toString();
            String hStr = heightInput.getText().toString();
            if (wStr.isEmpty() || hStr.isEmpty() || wStr.equals("0") || hStr.equals("0")) return;
            if (!canResize) {
    ...
    private void fetchAndSaveArtwork(int w, int h, String intentArtworkUrl, androidx.appcompat.app.AlertDialog dialog) {
        if (intentArtworkUrl == null) return;
        dialog.setCancelable(false);
        activity.showLoading("Fetching & Saving " + w + "x" + h + "...");
        String newUrl = intentArtworkUrl.replace("{w}", String.valueOf(w)).replace("{h}", String.valueOf(h)).replace("{f}", "jpg");

        Glide.with(activity).asBitmap().load(newUrl)
    ...
    public String detectMimeTypeFromBitmap(Bitmap bitmap) { return "image/png"; }

    public Bitmap getSelectedArtwork() { return selectedArtwork; }
    public byte[] getSelectedArtworkBytes() { return selectedArtworkBytes; }
    public String getSelectedArtworkMimeType() { return selectedArtworkMimeType; }
    public boolean isArtworkChanged() { return artworkChanged; }
}
